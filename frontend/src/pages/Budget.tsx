...
export default function Budget() {
  ...
  const mutation = useMutation({
    mutationFn: () => budgetApi.generatePlan(budget),
    onSuccess: (res) => {
      setPlan(res.data.plan);
      toast.success('Budget meal plan generated');
    },
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <GlassCard>
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <span className="font-display font-bold text-lg text-slate-800 dark:text-slate-100 w-20 text-right">${budget}</span>
        </div>
