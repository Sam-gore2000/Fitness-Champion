...
export const aiApi = {
  chat: (message: string) => api.post<{ reply: string; offline?: boolean }>('/ai/chat', { message }),
  ...
  estimateNutrition: (name: string, quantityValue: number, unit: string) =>
    api.post<{ nutrition: { calories: number; protein: number; carbs: number; fat: number; fiber: number; sugar: number }; quantityDescription: string; offline?: boolean }>(
